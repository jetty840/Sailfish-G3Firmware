...
void plan_dump_current_block(int discard, int report)
{
     ...
     if (block->acceleration_rate == 0 || !block->use_accel )
     {
	     // No acceleration
     ...
     else
     {
		  ...
		  step_events_completed <= block->step_event_count; )
	     {
		     if (step_events_completed <= (uint32_t)(0x7fffffff & block->accelerate_until))
		     {
			     ...
			     acc_step_rate += block->initial_rate;
			     if (report && acc_step_rate < old_acc_step_rate)
		     ...
		     else if (step_events_completed > (uint32_t)(0x7fffffff & block->decelerate_after))
		     {
			     ...
			     if (dec_step_rate < block->final_rate)
				     dec_step_rate = block->final_rate;
			     if (report && intermed < old_intermed)
     ...
     for (int j = 0; j < STEPPER_COUNT; j++)
     {
	     ...
	     if (count_direction[j] < 0)
		     s = -s;
	     speed = s * (float)initial_rate * 10000000000.0f / (float)eeprom::axis_steps_per_mm[j];
	     delta = fabs(speed - prev_speed[j]) - FPTOF(max_speed_change[j]);
	     if (report && delta > 0.1f)
	     {
	     ...
	     prev_speed[j] = s * (float)dec_step_rate * 10000000000.0f / (float)eeprom::axis_steps_per_mm[j];
     }
     if (maxd > 0.1f)
     {
     ...
     {
	  float delta_z = (float)block->steps[Z_AXIS] * 10000000000.0f / (float)eeprom::axis_steps_per_mm[Z_AXIS];
	 ...
	 if (simulator_dump_speeds)
	 {
	     ...
	     float dx = (float)block->steps[X_AXIS] * 10000000000.0f / (float)eeprom::axis_steps_per_mm[X_AXIS];
	     float dy = (float)block->steps[Y_AXIS] * 10000000000.0f / (float)eeprom::axis_steps_per_mm[Y_AXIS];
	     float dz = (float)block->steps[Z_AXIS] * 10000000000.0f / (float)eeprom::axis_steps_per_mm[Z_AXIS];
	     float speed_xyz = sqrt(dx*dx+dy*dy+dz*dz) / total_time;

	     printf("%d %s: z=%5.2f entry=%5u, peak=%5d, final=%5d steps/s; planned=%d; "
...
uint8_t getEeprom8Sub(const uint16_t location, const uint8_t default_value, const bool defaultCheck) { return default_value; }

uint8_t getEeprom8(const uint16_t location, const uint8_t default_value) { return default_value; }

uint8_t getEeprom8(const uint16_t location) { return (uint8_t)0; }
...
uint32_t getEepromUInt32(const uint16_t location, const uint32_t default_value) { return default_value; }
...
int64_t getEepromStepsPerMMSub(const uint16_t location, const int64_t default_value, const bool defaultCheck) { return default_value; }

int64_t getEepromStepsPerMM(const uint16_t location, const int64_t default_value) { return default_value; }

int64_t getEepromStepsPerMM(const uint16_t location) { return (int64_t)0; }

void setJettyFirmwareDefaults() { }
void storeToolheadToleranceDefaults() { }

uint64_t axis_steps_per_mm[5] = {
	EEPROM_DEFAULT_STEPS_PER_MM_X,
	EEPROM_DEFAULT_STEPS_PER_MM_Y,
	EEPROM_DEFAULT_STEPS_PER_MM_Z,
	EEPROM_DEFAULT_STEPS_PER_MM_A,
	EEPROM_DEFAULT_STEPS_PER_MM_B};
}
