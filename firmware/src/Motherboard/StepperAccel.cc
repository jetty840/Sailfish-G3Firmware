...
bool		extruder_hold[EXTRUDERS];		// True if the extruders should not be disabled during printing
	...
	static uint8_t			st_extruder_interrupt_rate[EXTRUDERS];
	static uint8_t			extruder_interrupt_steps_per_call[EXTRUDERS];
...
FORCE_INLINE void setup_next_block() {
	...
	if ( e_steps[0] || extruder_hold[0])	extruderOverriddenAxesEnabled |= _BV(A_AXIS);
#if EXTRUDERS > 1
	if ( e_steps[1] || extruder_hold[1])	extruderOverriddenAxesEnabled |= _BV(B_AXIS);
#endif
...
bool st_interrupt() {    
	...
	if (current_block == NULL) {
		...
		if ( deprime_enabled ) {
			for ( uint8_t e = 0; e < EXTRUDERS; e ++ ) {
				if ((( current_block == NULL ) ||
				     (! (current_block->axesEnabled & _BV(A_AXIS + e)))
				     #ifdef DEPRIME_ON_NO_EXTRUSION
				    	 || ( (current_block->steps[A_AXIS] == 0) 
						#if EXTRUDERS > 1
							&& (current_block->steps[B_AXIS] == 0)
						#endif
				     	    )
				     #endif
				    ) && ( ! deprimed[e] )) {
